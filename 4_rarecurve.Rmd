...
#rarefy
```{r}
assay(SEtest, "rarefied") <-
  SEtest %>%
  assay("counts") %>%
  t() %>%
  vegan::rrarefy(sample = 5000) %>%
  t()
```
# Load containers
...
curve <- rarecurve(otu_table, step = 20, sample = sorted_reads, col = "blue", cex = 0.6, tidy = TRUE)
ggplot(curve, aes(x = Sample, y = Species, group = Site))+
  geom_line(data = curve, aes(x = Sample, y = Species))+
  labs(x = "Reads", y = "ASVs")
# Create the ggplot
ggplot(rarefaction_df, aes(x = Sequences, y = ASVs)) +
  geom_line(size = 2, color = "nice_colors") +
  labs(x = "Reads", y = "ASVs") +
  theme_minimal()
```
...
rarecurve(otu_table, sample = sorted_reads, col="grey15", step = 1000, tidy = TRUE)
