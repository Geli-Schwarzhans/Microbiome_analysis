...
SE <- readRDS(file.path(RD_DIR, "SE_all_finalremovedSamples.rds"))
...
plot1 <- 
  ggplot(data = as.data.frame(colData(SEtest)), aes(x = Group, y = shannon, fill = Species)) +
  geom_boxplot() +
  geom_dotplot(binaxis='y', stackdir='center', position=position_dodge(), binwidth = .06)+
...
ggsave(file.path(PLOTS_DIR, g("{PROJECT_NAME}_abs_alpha_div_box-shannon.pdf")), width = 10, height = 8, limitsize = FALSE)
```
# Calculate using observed richness
```{r}
SEtest <- mia::estimateEvenness(SEtest, 
                       abund_values = "counts", 
                       index = "simpson_evenness", 
                       name="simpson")
head(colData(SEtest)$simpson)

plot2 <- ggplot(data = as.data.frame(colData(SEtest)), aes(x = Group, y = simpson, fill = Species)) +
  geom_boxplot() +
  geom_dotplot(binaxis='y', stackdir='center', position=position_dodge(), binwidth = 0.0035)+
  scale_fill_manual(values = alpha(c("steelblue3","orange2","grey15", "firebrick3"), .7)) +
  #scale_y_continuous(breaks =c(100,200, 300, 400,500, 600))+
  facet_grid(. ~ collection_date, scales="free") + 
    theme_bw() +  
    ...
    theme(legend.text = element_text(size=15))+
    theme(legend.key.size = unit(1.0, 'cm'))+
  ylab(expression(Evenness[simpson]))

ggsave(file.path(PLOTS_DIR, g("{PROJECT_NAME}_abs_alpha_div_box-observed.pdf")), width = 10, height = 8, limitsize = FALSE)
```
...
SEtest <-estimateDiversity(SEtest,
  abund_values = "counts",
  index = "inverse_simpson",
  )

SEtest <-estimateDiversity(
  SEtest,
  assay.type = "counts",
  assay_name = NULL,
  index ="inverse_simpson"#,
...
plot3 <- 
ggplot(data = as.data.frame(colData(SEtest)), aes(x = Group, y = inverse_simpson, fill = Species)) +
  geom_boxplot() +
...
ggsave(file.path(PLOTS_DIR, g("{PROJECT_NAME}_abs_alpha_div_box-simpson.svg")), width = 8, height = 4, limitsize = FALSE)
```
...
plot4 <- ggplot(data = as.data.frame(colData(SEtest)), aes(x = Group, y = chao1, fill = Species)) +
  geom_boxplot() +
  geom_dotplot(binaxis='y', stackdir='center', position=position_dodge(), binwidth = 9.5)+
  scale_fill_manual(values = alpha(c("steelblue3","orange2","grey15", "firebrick3"), .7)) +
  scale_y_continuous(breaks =c(100,200, 300, 400,500, 600))+
  facet_grid(. ~ collection_date, scales="free") + 
    theme_bw() +  
...
ggsave(file.path(PLOTS_DIR, g("{PROJECT_NAME}_abs_alpha_div_box-chao1.pdf")), width = 10, height = 8, limitsize = FALSE)
```
# Calculate using observed richness
```{r}
SEtest <- mia::estimateRichness(SEtest, 
...
plot5 <- ggplot(data = as.data.frame(colData(SEtest)), aes(x = Group, y = observed, fill = Species)) +
  geom_boxplot() +
  geom_dotplot(binaxis='y', stackdir='center', position=position_dodge(), binwidth = 8.5)+
  scale_fill_manual(values = alpha(c("steelblue3","orange2","grey15", "firebrick3"), .7)) +
  #scale_y_continuous(breaks =c(100,200, 300, 400,500, 600))+
  facet_grid(. ~ collection_date, scales="free") + 
    theme_bw() +  
...
plot6 <- ggplot(data = as.data.frame(colData(SEtest)), aes(x = Group, y = relative, fill = Species)) +
  geom_boxplot() +
  geom_dotplot(binaxis='y', stackdir='center', position=position_dodge(), binwidth = .01)+
  scale_fill_manual(values = alpha(c("steelblue3","orange2","grey15", "firebrick3"), .7)) +
  #scale_y_continuous(breaks =c(100,200, 300, 400,500, 600))+
  facet_grid(. ~ collection_date, scales="free") + 
    theme_bw() +  
    ...
    theme(legend.text = element_text(size=15))+
    theme(legend.key.size = unit(1.0, 'cm'))+
  ylab(expression(Dominance[relative]))

ggsave(file.path(PLOTS_DIR, g("{PROJECT_NAME}_abs_alpha_div_box-relative.pdf")), width = 10, height = 8, limitsize = FALSE)
```

## Combine them all in one plot
```{r}
grid_plot <- grid.arrange(plot1,plot2, plot3, plot4, plot5, plot6, ncol = 3)
ggsave(plot=grid_plot, file.path(PLOTS_DIR, g("{PROJECT_NAME}_abs_alpha_div_box-all.svg")), width = 15, height = 5, limitsize = FALSE)
```
...
ggsave(file.path(PLOTS_DIR, g("{PROJECT_NAME}_abs_alpha_div_box-faith.pdf")), width = 10, height = 8, limitsize = FALSE)
```
...
for (i in seq_along(timepoints)) {
  for (subset in sample_subsets) {
    ...
    # Perform the kruskal/wilcox
    #kruskal_result <- kruskal.test(inverse_simpson ~ Species, data = subset_data)
    wilcox_result <- pairwise.wilcox.test(subset_data$inverse_simpson, subset_data$Species, p.adjust.method = "BH")
    pmatrix<-wilcox_result$p.value
    dt_long <- as.data.table(as.table(pmatrix))
    setnames(dt_long, c("Species1", "Species2", "p_value"))
    dt_long <- dt_long[!is.na(p_value)]
    pval_matrix <- as.matrix(dt_long[, .(p_value = as.numeric(p_value))])
    pval_matrix
    ...
    wilcox_pvalue <- pval_matrix[, "p_value"][1]
    ...
    results_time <- rbind(results_time, data.table(Timepoint = timepoints[i], Species1 = subset[1], Species2 = subset[2], Wilcox_pvalue = wilcox_pvalue, Anova_pvalue = anova_pvalue, TukeyHSD_pvalue = tukeyhsd_pvalue, TukeyHSD_sig = sig), fill = TRUE)
  }
}
...
```{r}
plots <- lapply(c("observed", "shannon", "relative","simpson"),
                plotColData,
                object = SEtest,
                x = "Group",
...
ggpubr::ggarrange(plotlist = plots, nrow = 2, ncol = 2, common.legend = TRUE, legend = "right")
```
...
alpha_diversity <- colData(SEtest) %>%
